...
void DecoderBase::SeekToPosition(float position) {
    LOGE("DecoderBase::SeekToPosition position=%f", position);
    std::unique_lock<std::mutex> lock(m_Mutex);
...
int DecoderBase::InitFFDecoder() {
    ...
    do {
        ...
        if(avformat_find_stream_info(m_AVFormatContext, nullptr) < 0) {
            LOGE("DecoderBase::InitFFDecoder avformat_find_stream_info fail.");
            break;
        }
...
int DecoderBase::DecodeOnePacket() {
    ...
    if(m_SeekPosition > 0) {
        ...
        if (seek_ret < 0) {
            m_SeekSuccess = false;
            LOGE("BaseDecoder::DecodeOneFrame error while seeking m_MediaType=%d", m_MediaType);
        } else {
            if (-1 != m_StreamIndex) {
